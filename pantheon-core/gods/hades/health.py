...
Phase 1 of the pipeline. Walks every known Codex and reports:
  ...
  - vector backend vs filesystem consistency (orphans in either direction)
...
def _walk_codex_files(codex_root: Path) -> Dict[str, List[Path]]:
    ...
    Returns: {
        ...
        "embeddable": [files that should be in the vector backend],
...
def run_health_checks() -> Dict[str, Any]:
    ...
    # Legacy vector-store counters are no longer tracked here.
    ...
    for codex_name in KNOWN_CODEXES:
        ...
        # Vector-vs-FS comparison removed in P4a.
        # Orphan detection is now file-system-only.
        health["chroma_vs_fs"][codex_name] = {
            "chroma": 0,
            "fs_embeddable": 0,
            "delta": 0,
            "note": "Vector comparison removed in P4a; comparison no longer meaningful",
        }
