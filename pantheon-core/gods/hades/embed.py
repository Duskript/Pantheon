"""
Hades embed module — retained as a no-op compatibility layer.

This module used to provide the embedding pipeline. Retrieval now uses FTS5
keyword search (see athenaeum_search MCP tool) and
...
# Reference to the legacy embedding directory — kept for diagnostic logging only
...
def _partition_for(codex: str) -> str:
    """No-op stub. Returns the codex name lowercased (matches legacy format)."""
...
def _recheck_embedded_counts(report: Any) -> None:  # noqa: ARG001
    """No-op stub. Embedding counts no longer relevant."""
    pass
...
def embed_missing_files(max_files: int | None = None) -> Dict[str, Any]:  # noqa: ARG001
    ...
    return {
        ...
        "embedded": 0,
        "errors": [],
        "note": "embed_missing_files is a no-op since P4a. "
