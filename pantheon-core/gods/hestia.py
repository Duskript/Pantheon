...
class HestiaChecker:
    ...
    def check_vector_backend(self) -> HealthStatus:
        """Check the sqlite-vec / event_embeddings backend used by Ichor."""
        try:
            from lib.ichor.vector_backend import VectorBackend
            t0 = time.monotonic()
            backend = VectorBackend(db_path=Path(f"{_HOME}/.hermes/ichor.db"))
            ok = backend.health()
            total_vecs = backend.count() if ok else 0
            latency_ms = (time.monotonic() - t0) * 1000.0
            return HealthStatus(
                service="vector-backend",
                ok=ok,
                latency_ms=round(latency_ms, 2),
                error=f"({total_vecs} vectors)" if ok and total_vecs > 0 else None,
            )
        except Exception as exc:
            return HealthStatus(service="vector-backend", ok=False, error=str(exc))
