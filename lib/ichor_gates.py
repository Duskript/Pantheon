...
class ForgeLogger:
    ...
    def log_intervention(
        ...
        # Skip writing the per-model file when model is the placeholder.
        # When model="unknown" for every event (current state of validation
        # probes), the per-model file would byte-identically duplicate
        # all.jsonl — wasted I/O and confusing for operators. all.jsonl
        # still captures the data. (Fix 2026-06-29: dedup log files.)
        model_file = (
            os.path.join(self.log_dir, f"{model_safe}.jsonl")
            if model and model != "unknown" else ""
        )
