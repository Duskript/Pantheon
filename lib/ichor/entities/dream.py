...
def _find_overlapping_windows(
    ...
    *,
    ...
    Returns: list of {r1_id, r2_id, type_id, source_id, target_id,
                      ...
                      r2_valid_to, reason}
    """
    clause_a, params_a = _temporal_clause(
        "a", point_in_time=point_in_time, include_historical=include_historical,
    )
    clause_b, params_b = _temporal_clause(
        "b", point_in_time=point_in_time, include_historical=include_historical,
    )
    rows = con.execute(
...
def detect_contradictions(
    con: sqlite3.Connection, *, dry_run: bool = False, point_in_time: str | None = None, include_historical: bool = False
) -> dict[str, Any]:
    ...
    """
    rel_conflicts = _find_overlapping_windows(
        con, point_in_time=point_in_time, include_historical=include_historical,
    )
    fact_conflicts = _find_value_conflicts(
        con, point_in_time=point_in_time, include_historical=include_historical,
    )
    return {
...
def _relationships_to_decay(
    ...
    rows = con.execute(
        ...
        "WHERE r.weight > 0 AND r.valid_to IS NULL"
    ).fetchall()
    return [dict(r) for r in rows]
