...
CREATE TABLE IF NOT EXISTS entities (
    ...
    -- Phase 1 (Validated-Only Graph): declared inline so the partial
    -- indexes below can be created in the same executescript() call.
    -- The same column is also added via ALTER TABLE in _MIGRATIONS for
    -- backwards compat with older DBs that pre-date Phase 1; the
    -- migration is idempotent (skips if the column already exists).
...
-- Phase 1 (Validated-Only Graph): partial index on validated rows so the
-- default `WHERE e.provisional = 0` filter in `traverse()` / `graph_query()`
-- uses an index seek instead of a full scan. `WHERE provisional = 0` is the
-- hot path; the inverse (`WHERE provisional = 1`) is rare (opt-in only).
CREATE INDEX IF NOT EXISTS idx_entities_validated ON entities(provisional) WHERE provisional = 0;
...
CREATE TABLE IF NOT EXISTS relationships (
    ...
    updated_at TEXT DEFAULT (datetime('now')),
    -- Phase 1 (Validated-Only Graph): declared inline so the partial
    -- index below can be created in the same executescript() call.
    -- Backwards compat for older DBs is handled by _MIGRATIONS.
    provisional INTEGER NOT NULL DEFAULT 0,
...
-- Phase 1 (Validated-Only Graph): partial index on validated edges so the
-- `WHERE r.provisional = 0` filter in the recursive CTE uses an index seek.
-- Same rationale as idx_entities_validated above.
CREATE INDEX IF NOT EXISTS idx_relationships_validated ON relationships(provisional) WHERE provisional = 0;
...
def migrate(db_path: Path | str | None = None) -> dict:
    ...
    try:
        ...
        # Phase 1 (Validated-Only Graph): partial indexes on
        # `provisional = 0` for fast filtered traversals. Must run
        # AFTER `_apply_column_migrations` since the indexes reference
...
#
# Phase 1 (Validated-Only Graph): partial indexes on `provisional = 0`
# rows so the default `WHERE e.provisional = 0` / `r.provisional = 0`
# filter in `traverse()` / `graph_query()` uses an index seek instead
# of a full scan. The `WHERE provisional = 0` predicate is the hot
# path; the inverse is rare (opt-in only). These indexes MUST run
# AFTER the column migrations above, since they reference `provisional`.
# Using `IF NOT EXISTS` makes them idempotent — re-runs on existing
...
# Phase 1 (Validated-Only Graph): partial indexes. Must run AFTER the
# column-level migrations above (because they reference `provisional`).
# Keyed by (table, name) so we can detect prior application idempotently.
_PHASE1_INDEX_DDL: list[dict] = [
    {
        "table": "entities",
        "name": "idx_entities_validated",
        "ddl": "CREATE INDEX IF NOT EXISTS idx_entities_validated "
               "ON entities(provisional) WHERE provisional = 0",
    },
    {
        "table": "relationships",
        "name": "idx_relationships_validated",
        "ddl": "CREATE INDEX IF NOT EXISTS idx_relationships_validated "
               "ON relationships(provisional) WHERE provisional = 0",
    },
]
...
def _apply_phase1_indexes(conn: sqlite3.Connection) -> list[str]:
    """Run the Phase 1 partial-index DDL if the index doesn't exist yet.

    Idempotent: queries `sqlite_master` for the index name and skips
    if found. Kept separate from `_apply_column_migrations` so the
    column migrations stay focused on ALTER TABLE.
    """
    applied: list[str] = []
    for ix in _PHASE1_INDEX_DDL:
        name = ix["name"]
        row = conn.execute(
...
_EXPECTED_INDEXES: dict[str, list[str]] = {
    ...
    "entities": [
        ...
        # Phase 1 (Validated-Only Graph): partial index on validated rows.
        "idx_entities_validated",
    ],
    ...
    "relationships": [
        ...
        # Phase 1 (Validated-Only Graph): partial index on validated edges.
        "idx_relationships_validated",
    ],
