...
# ---------- Temporal supersession helpers ----------

def supersede_if_duplicate(
    conn: sqlite3.Connection,
    table: str,
    match_keys: dict[str, Any],
    new_row: dict[str, Any],
) -> list[int]:
    """Close open relationship rows before inserting a successor.

    The current schema branch only exposes temporal validity on
    `relationships`, so this helper is relationship-scoped for now.
    Callers insert the new row in the same transaction after this
    returns.
    """
    if table != "relationships":
        raise ValueError(
            f"supersede_if_duplicate currently supports relationships only, got {table!r}"
        )

    type_id = match_keys.get("type_id")
    source_id = match_keys.get("source_id")
    target_id = match_keys.get("target_id")
    if type_id is None or source_id is None or target_id is None:
        raise ValueError("match_keys must include type_id, source_id, and target_id")

    new_valid_from = new_row.get("valid_from") or match_keys.get("valid_from")
    sql = (
        "SELECT id FROM relationships "
        "WHERE type_id = ? AND source_id = ? AND target_id = ? "
        "  AND valid_to IS NULL"
    )
    params: list[Any] = [type_id, source_id, target_id]
    if new_valid_from is not None:
        sql += " AND (valid_from IS NULL OR valid_from <> ?)"
        params.append(new_valid_from)
    ...
    conn.execute(
        f"UPDATE relationships SET valid_to = datetime('now'), updated_at = datetime('now') WHERE id IN ({placeholders})",
        ids,
    )
    return ids
...
def backfill_relationship_temporal_bounds(
    conn: sqlite3.Connection,
    *,
    dry_run: bool = False,
) -> dict[str, Any]:
    """Best-effort relationship temporal backfill.

    For each `(type_id, source_id, target_id)` chain, if an older row
    still has `valid_to IS NULL` and a newer successor exists, set the
    older row's `valid_to` to the successor's `valid_from` (or its
    `created_at` / now fallback when the successor lacks a timestamp).

    This is intentionally conservative: it only updates rows that are
    currently open-ended and have a clear successor.
    """
    conn.row_factory = sqlite3.Row
    rows = conn.execute(
        """SELECT id, type_id, source_id, target_id, valid_from, valid_to, created_at
           FROM relationships
           ORDER BY type_id, source_id, target_id,
                    COALESCE(valid_from, created_at), id"""
    ).fetchall()

    groups: dict[tuple[Any, Any, Any], list[sqlite3.Row]] = {}
    for row in rows:
        groups.setdefault((row["type_id"], row["source_id"], row["target_id"]), []).append(row)

    groups_scanned = 0
    rows_updated = 0
    rows_already_closed = 0

    for chain in groups.values():
        if len(chain) < 2:
            continue
        groups_scanned += 1
        for idx, current in enumerate(chain[:-1]):
            if current["valid_to"] is not None:
                rows_already_closed += 1
                continue
            successor = chain[idx + 1]
            new_valid_to = successor["valid_from"] or successor["created_at"] or None
            if not dry_run:
                if new_valid_to is None:
                    conn.execute(
                        "UPDATE relationships SET valid_to = datetime('now'), updated_at = datetime('now') WHERE id = ?",
                        (current["id"],),
                    )
                else:
                    conn.execute(
                        "UPDATE relationships SET valid_to = ?, updated_at = datetime('now') WHERE id = ?",
                        (new_valid_to, current["id"]),
                    )
            rows_updated += 1
    ...
    return {
        ...
        "rows_already_closed": rows_already_closed,
        "dry_run": dry_run,
    }
