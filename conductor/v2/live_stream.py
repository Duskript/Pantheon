    ...
    async def broadcast(self, event: StreamEvent) -> int:
        ...
        if listeners:
            ...
            for q in listeners:
                ...
                # Overflow policy: drop the OLDEST event to keep the
                # stream showing the latest state. For a live event
                # stream, recency matters more than a 1024-event
                # historical window. If the drain+put race fails (a
                # wedged TCP socket), fall through to a warning drop
                ...
                try:
                    ...
                    try:
                        ...
                        LOG.debug(
                            ...
                            f"dropped oldest, kept latest {event.event!r}"
                        )
                    except (asyncio.QueueEmpty, asyncio.QueueFull):
                        LOG.warning(
                            f"workflow listener queue wedged for "
                            f"workflow_id={event.workflow_id} — "
                            f"event {event.event!r} dropped"
                        )
