...
# ── God roster response cache ───────────────────────────────────────────────
# /api/gods can take 20+ s on cold runs (hermes_cli.profiles.list_profiles is
...
<a href="/">← Back to Pantheon</a>
    ...
    # ── Iris handoff — Wave 3: new operations surface adapters ──────────
    if parsed.path.startswith("/api/briefing"):
        from api.briefing_bridge import handle_briefing
        result = handle_briefing(handler, parsed)
        if result is False:
            return bad(handler, "Unknown briefing endpoint", status=404)
        return True

    if parsed.path.startswith("/api/channels"):
        from api.channels_bridge import handle_channels
        result = handle_channels(handler, parsed)
        if result is False:
            return bad(handler, "Unknown channels endpoint", status=404)
        return True

    if parsed.path.startswith("/api/orchestration"):
        from api.orchestration_bridge import handle_orchestration
        result = handle_orchestration(handler, parsed)
        if result is False:
            return bad(handler, "Unknown orchestration endpoint", status=404)
        return True

    if parsed.path.startswith("/api/files/"):
        from api.files_bridge import handle_files
        result = handle_files(handler, parsed)
        if result is False:
            return bad(handler, "Unknown files endpoint", status=404)
        return True

    if parsed.path == "/api/work/interventions":
        from api.interventions_bridge import handle_interventions
        result = handle_interventions(handler, parsed)
        if result is False:
            return bad(handler, "Unknown interventions endpoint", status=404)
        return True

    if parsed.path in ("/api/work/lanes", "/api/work/ideas", "/api/work/cron",
                       "/api/work/failures", "/api/workflows/runs"):
        from api.work_bridge import handle_work
        result = handle_work(handler, parsed)
        if result is False:
            return bad(handler, "Unknown work endpoint", status=404)
        return True

    if parsed.path in ("/api/control/plugins", "/api/control/skills"):
        from api.control_bridge import handle_control
        result = handle_control(handler, parsed)
        if result is False:
            return bad(handler, "Unknown control endpoint", status=404)
        return True

    if parsed.path == "/api/control/integrations":
        from api.integrations_bridge import handle_integrations
        result = handle_integrations(handler, parsed)
        if result is False:
            return bad(handler, "Unknown integrations endpoint", status=404)
        return True
    ...
    if parsed.path == "/api/projects":
        # Cached — load_projects() can be slow on cold runs.
        global _PROJECTS_CACHE, _PROJECTS_CACHE_TS
        now = time.time()
        if _PROJECTS_CACHE is not None and now - _PROJECTS_CACHE_TS < 30:
            return j(handler, _PROJECTS_CACHE)
    ...
    if parsed.path == "/api/gods":
        ...
        # Defensive: warn on duplicate display_names so the UI doesn't
        # show two "Hephaestus" cards. Surfaced as `display_name_warnings`
        # in the payload — the frontend can choose to dedupe or badge.
        seen: dict[str, str] = {}
        warnings: list[dict] = []
        for g in gods:
            dn = g.get("display_name") or ""
            if dn and dn in seen:
                warnings.append({"display_name": dn, "names": [seen[dn], g["name"]]})
            elif dn:
                seen[dn] = g["name"]
...
def _handle_sse_stream(handler, parsed):
    ...
    # Allow the Vite dev UI on the same host but a different port to attach
    # directly to SSE. The Vite proxy can keep EventSource/fetch streams open
    # after the backend has flushed buffered events, leaving the composer stuck.
    try:
        origin = handler.headers.get("Origin", "")
        host = handler.headers.get("Host", "")
        if origin:
            m = _re.match(r"^https?://([^/:]+)(?::\d+)?", origin)
            origin_name = m.group(1).lower() if m else ""
            host_name = host.split(":", 1)[0].lower()
            if origin.rstrip("/").lower() in _allowed_public_origins() or (origin_name and origin_name == host_name):
