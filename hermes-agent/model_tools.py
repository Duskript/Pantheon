...
def _emit_post_tool_call_hook(
    *,
    ...
    # Active model + provider for the turn that produced this tool call.
    # Downstream hooks (e.g. ichor-gates) use these to attribute
    # interventions to the correct model — fixes the "model: unknown"
    # gap in forge logs. (Added 2026-06-29.)
...
def handle_function_call(
    ...
    enabled_toolsets: Optional[List[str]] = None,
    disabled_toolsets: Optional[List[str]] = None,
    # Active model + provider for the current turn. Threaded to
    # _emit_post_tool_call_hook so plugins (ichor-gates, etc.) can
    # attribute interventions to the right model. Optional — falls back
    # to whatever the plugin can derive from its own context.
    # (Added 2026-06-29.)
    model: Optional[str] = None,
    provider: Optional[str] = None,
) -> str:
    ...
    try:
        ...
        try:
            from hermes_cli.plugins import has_hook, invoke_hook
            # TokenJuice: pass the active profile as the `god` kwarg so
            # compression-attribution hooks know which god to bill.
            try:
