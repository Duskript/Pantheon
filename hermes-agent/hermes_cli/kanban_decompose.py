...
try:
    # HephaestusBuildCard contract — see hermes_cli/kanban_contract.py.
    # Imported lazily and tolerantly: if the contract module is missing
    # (older hermes_cli install) we just skip the new validation path.
    from hermes_cli.kanban_contract import (  # type: ignore
        HephaestusBuildCard,
        detect_conflicts as _detect_card_conflicts,
        validate_card as _validate_card,
    )
    _CONTRACT_AVAILABLE = True
except Exception as _exc:  # ImportError or syntax error in old envs
    logger.debug("decompose: contract import unavailable: %s", _exc)
    HephaestusBuildCard = None  # type: ignore
    _detect_card_conflicts = None  # type: ignore
    _validate_card = None  # type: ignore
...
# ---------------------------------------------------------------------------
# Contract helpers — soft validation for card-shaped children.
# ---------------------------------------------------------------------------


def _is_card_shape(child: dict) -> bool:
    """Return True iff ``child`` looks like a HephaestusBuildCard.

    A card has a ``card_id`` field matching the contract pattern. Legacy
    LLM decompose children (title/body/assignee/parents) do not.
    """
    if not isinstance(child, dict):
        return False
    cid = child.get("card_id")
    return isinstance(cid, str) and cid.startswith("hep-")


def _validate_card_children(children: list[dict]) -> list[str]:
    """Run every card-shaped child through ``validate_card``.

    Returns a list of human-readable errors. Empty list = all valid.
    """
    errors: list[str] = []
    for i, child in enumerate(children):
        is_valid, card_errors = _validate_card(child)
        if not is_valid:
            label = child.get("card_id") or child.get("title") or f"#{i}"
            errors.append(
                f"Child {label}: " + "; ".join(card_errors)
            )
    return errors


def _auto_inject_conflicts(children: list[dict]) -> int:
    """Run conflict detection on card-shaped children and inject conflicts_with.

    Returns the number of conflict pairs detected (for logging).
    """
    cards: list[HephaestusBuildCard] = []
    for c in children:
        if not _is_card_shape(c):
            continue
        try:
            cards.append(HephaestusBuildCard(**c))
        except (TypeError, KeyError) as exc:
            logger.info("decompose: card init failed for %s: %s", c.get("card_id"), exc)
            continue

    if not cards:
        return 0

    auto_conflicts = _detect_card_conflicts(cards)
    for a_id, b_id, reason in auto_conflicts:
        for c in children:
            if c.get("card_id") != a_id:
                continue
            cw = c.setdefault("conflicts_with", [])
            if b_id not in cw:
                cw.append(b_id)
        for c in children:
            if c.get("card_id") != b_id:
                continue
            cw = c.setdefault("conflicts_with", [])
            if a_id not in cw:
                cw.append(a_id)
        logger.info("decompose: auto-conflict %s ↔ %s: %s", a_id, b_id, reason)

    return len(auto_conflicts)
...
def decompose_task(
    ...
    *,
    ...
    # Contract validation — only kicks in when children are card-shaped.
    # Legacy LLM decompose output (title/body/assignee/parents) is unaffected.
    # The new spec-to-cards skill emits card-shaped children; this gate
    # aborts the decompose before any card is written if validation fails.
    has_card_children = any(_is_card_shape(t) for t in raw_tasks)
    if has_card_children and _CONTRACT_AVAILABLE:
        card_errors = _validate_card_children(raw_tasks)
        if card_errors:
            msg = (
                f"Decompose aborted: {len(card_errors)} child card(s) failed "
                f"contract validation:\n  - "
                + "\n  - ".join(card_errors)
            )
            logger.warning("decompose: %s", msg)
            return DecomposeOutcome(task_id, False, msg)
        n_conflicts = _auto_inject_conflicts(raw_tasks)
        logger.info(
            "decompose: contract validation passed for %d card-shaped "
            "children (%d auto-conflicts injected)",
            sum(1 for t in raw_tasks if _is_card_shape(t)),
            n_conflicts,
        )
    elif has_card_children and not _CONTRACT_AVAILABLE:
        logger.warning(
            "decompose: %d card-shaped children detected but contract "
            "module unavailable — proceeding without validation. "
            "Update hermes_cli to enable contract enforcement.",
            sum(1 for t in raw_tasks if _is_card_shape(t)),
        )
