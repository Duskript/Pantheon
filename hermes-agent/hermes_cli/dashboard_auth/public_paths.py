...
PUBLIC_API_PATHS: frozenset[str] = frozenset({
    ...
    "/api/cron/fire",
    # Pantheon Pairing: bootstrap auth endpoints — no session, no cookie.
    # POST /pair generates a one-time code; POST /exchange swaps it for a
    # session. Both are the bootstrap flow; gating them would deadlock.
    "/api/plugins/pantheon-pairing/pair",
    "/api/plugins/pantheon-pairing/exchange",
})
