...
def _get_active_model() -> str:
    """Best-effort: read the active model from the god's config.yaml.

    Used as a fallback when the post_tool_call hook doesn't pass model=
    (e.g. internal sandbox-to-sandbox bridges, older call paths). Reads
    ~/.hermes/profiles/<god>/config.yaml OR ~/.hermes/config.yaml and
    returns `model.default` (the resolved active model) or "" on any
    error. Cheap (single yaml.safe_load, cached after first read).
    """
    global _cached_active_model
    if _cached_active_model is not None:
        return _cached_active_model
    try:
        # Walk up from HERMES_HOME to find a config.yaml
    ...
    candidates = [
        home / "config.yaml",
        home.parent / "config.yaml" if home.name != ".hermes" else None,
    ]
    for cfg_path in candidates:
        ...
        try:
            ...
            if model:
                _cached_active_model = str(model)
                return _cached_active_model
        except Exception:
            continue
        ...
        if tool_name in _WRITE_TOOLS:
            ...
            for gr in gate_results:
                if not gr.passed and gr.intervention:
                    ...
                    # Log to Forge
                    # Attribute to the model that produced this tool call.
                    # Priority: explicit kwarg from hook → cached active model
                    # from config.yaml → "unknown" (last resort). Previously
                    # this always fell back to "unknown" because the hook
                    # payload didn't include model info — fix 2026-06-29.
                    model = (
                        kwargs.get("model")
                        or _get_active_model()
                        or "unknown"
                    )
