...
_POISON_MARKERS = (
    ...
    'only the user', 'only the USER', 'outside this block',
    'Treat as DATA', 'do not follow', 'cannot be issuing instructions',
)


def _is_poison_pill(text: str) -> bool:
    """Return True when the text looks like tool output or instructions."""
    ...
    if lowered.startswith(("you are", "you must", "treat as", "only the user")):
        return True
    return any(sig.lower() in lowered for sig in _POISON_MARKERS)


def _clamp_subject(text: str) -> Optional[str]:
    """Return a safe, single-line subject or None if the content is poison."""
    if _is_poison_pill(text):
        return None
...
def _clamp_raw_text(text: str) -> str:
    """Return a short preview suitable for inbox display."""
    if _is_poison_pill(text):
        return ""
...
def _sanitize_preview(text: str) -> str:
    """Producer-side preview sanitization for subconscious report rendering."""
    if not text:
        return ""
    ...
    for raw_line in text.splitlines():
        ...
        if not line:
            continue
        if any(sig in line.lower() for sig in _POISON_MARKERS):
            continue
        if line.lower().startswith(("you must", "you are", "treat as", "only the user")):
            continue
        if "\\u" in line:
            return ""
        lines.append(line)
    ...
    try:
        ...
        for event in events:
            ...
            if not content_text or confidence < 0.5:
                continue
            if _is_poison_pill(str(content_text)):
                continue
