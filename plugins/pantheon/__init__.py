...
Provides persistent, Codex-partitioned memory via an embedded Zvec vector
...
Architecture:
  ...
  Mnemosyne — embedded Zvec store holding semantic embeddings for Athenaeum
    content. Codex-scoped by collection. Rebuilt if corrupted.

  Vault — real-time session logging to the Athenaeum. Each turn is written
    immediately to avoid data loss.

Lifecycle:
  initialize() — connect to Zvec, set up paths, warm embeddings
...
_DEFAULT_ZVEC_DIR = _os.path.expanduser("~/.hermes/pantheon/zvec")
...
def _load_config(hermes_home: str) -> dict:
    ...
    for key in ("athenaeum_root", "zvec_dir"):
        raw = config.get(key, "")
        if raw:
            try:
                p = Path(raw)
...
def _partition_for(codex: str) -> str:
    """Return the Zvec collection name for a Codex tag.
...
def _doc_id_for_path(path: Path) -> str:
    """Stable, Zvec-safe document id for a file path."""
...
class PantheonMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._zvec: Any = None  # zvec collection/cache root
        ...
        self._zvec_root: Optional[Path] = None
        self._zvec_collections: Dict[str, Any] = {}
        self._zvec_dims: Dict[str, int] = {}
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        return [
            ...
            {
                "key": "zvec_dir",
                "description": "Path for embedded Zvec data",
                "default": _DEFAULT_ZVEC_DIR,
                "required": False,
            },
            {
                "key": "vault_codex",
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        Sets up the Athenaeum path, connects to Zvec, prepares the
        ...
        zvec_dir = Path(self._config["zvec_dir"])
        ...
        # Set up Zvec — embedded, no Docker needed
        try:
            import zvec as _zvec  # noqa: PLC0415

            self._zvec = _zvec
            self._zvec_root = zvec_dir
            self._zvec_available = True
        except Exception as exc:
            logger.warning("Zvec initialization failed: %s", exc)
            self._zvec_available = False
        ...
        # Warm up: ensure Zvec roots exist for all Codexes we can currently see
        if self._zvec_available:
    ...
    def shutdown(self) -> None:
        """Clean shutdown — flush vault, close Zvec. (Graph removed P4b.)"""
        if self._vault:
    ...
    def _ensure_codex_collections(self) -> None:
        """Ensure the Zvec root exists for visible Codex partitions."""
        if not self._zvec_available or not self._athenaeum_root or not self._zvec_root:
            return
        self._zvec_root.mkdir(parents=True, exist_ok=True)
        for codex in _list_codexes(self._athenaeum_root):
            partition = _partition_for(codex)
            self._zvec_collections.setdefault(partition, None)

    def _get_collection(self, codex: str, dim: int) -> Any:
        """Get or create a Zvec collection for *codex* and vector dimension."""
        if not self._zvec_available or not self._zvec_root or not self._zvec:
            return None
        partition = _partition_for(codex)
        cached = self._zvec_collections.get(partition)
        if cached is not None and self._zvec_dims.get(partition) == dim:
            return cached
        try:
            schema = self._zvec.CollectionSchema(
                name=partition,
                fields=[
                    self._zvec.FieldSchema("source", self._zvec.DataType.STRING, nullable=True),
                    self._zvec.FieldSchema("codex", self._zvec.DataType.STRING, nullable=True),
                    self._zvec.FieldSchema("filename", self._zvec.DataType.STRING, nullable=True),
                ],
            ...
            collection = self._zvec.create_and_open(
                path=str(self._zvec_root / partition),
                schema=schema,
            )
            self._zvec_collections[partition] = collection
            self._zvec_dims[partition] = dim
            return collection
        except Exception as exc:
            logger.warning("Failed to get Zvec collection for %s: %s", codex, exc)
            return None

    def _scoped_collections(self, scope: Optional[List[str]] = None) -> List[str]:
        """Return collection names in scope."""
        if not self._athenaeum_root:
            return []
        all_cols = [_partition_for(c) for c in _list_codexes(self._athenaeum_root)]
    ...
    def embed_file(self, file_path: str, codex: str) -> bool:
        ...
        if not self._zvec_available or not self._embedder:
            return False
        ...
        try:
            ...
            collection = self._get_collection(codex, len(embedding))
    ...
    def embed_athenaeum(self) -> Tuple[int, int]:
        ...
        """
        if not self._athenaeum_root or not self._zvec_available or not self._embedder:
            return (0, 0)
    ...
    def query(
        ...
        """
        if not self._zvec_available or not self._embedder or not self._zvec_root:
            return []
        ...
        for name in collection_names:
            partition_path = self._zvec_root / name
            if not partition_path.exists():
                continue
            try:
                collection = self._get_collection(_codex_from_partition(name), len(embedding))
                if collection is None:
                    continue
                raw = collection.query(
                    self._zvec.Query("embedding", vector=embedding),
                    topk=n_results,
                )
            except Exception as exc:
            ...
            for item in raw:
                ...
                if not doc_id or doc_id in seen:
                    continue
                fetched = collection.fetch(ids=[doc_id])
                fetched_doc = fetched.get(doc_id) if isinstance(fetched, dict) else None
                fields = getattr(fetched_doc, "fields", {}) if fetched_doc else {}
                source = fields.get("source", "") if isinstance(fields, dict) else ""
                if not source:
                    continue
                ...
                try:
                    ...
                    content = ""
                if not content and not doc_path.exists():
                    continue
                try:
                    rel = doc_path.resolve().relative_to(self._athenaeum_root.resolve())
                    codex = rel.parts[0] if rel.parts else _codex_from_partition(name)
                except Exception:
                    codex = _codex_from_partition(name)
                seen.add(doc_id)
                results.append({
    ...
    def remove_stale(self, file_path: str) -> bool:
        """Remove a file's embedding from Zvec. Used when files are archived."""
        if not self._zvec_available:
            return False
        doc_id = _doc_id_for_path(Path(file_path))
        try:
            removed = False
            for partition, collection in list(self._zvec_collections.items()):
                if collection is None:
                    continue
                try:
            ...
            if self._athenaeum_root:
                try:
                    rel = Path(doc_id).resolve().relative_to(self._athenaeum_root.resolve())
                    codex = rel.parts[0] if rel.parts else ""
                    dim = self._zvec_dims.get(_partition_for(codex))
                    if codex and dim:
                        collection = self._get_collection(codex, dim)
