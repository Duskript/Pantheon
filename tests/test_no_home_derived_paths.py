...
four unrelated tests elsewhere in the session. (Cost an hour to find; it is the
same family as the reload/attribute-patch finding.)
...
#: (module, attribute) for every site fixed for this defect.
SITES = [
...
def test_no_path_constant_follows_a_sandboxed_home(tmp_path):
    """The whole point: a god session must resolve the SAME paths as everyone else."""
...
def test_every_listed_site_produced_a_value(tmp_path):
    ...
    sandbox.mkdir(parents=True)
    values = _probe_with_home(sandbox)
    assert len(values) == len(SITES), set(SITES) ^ {tuple(k.split(".")) for k in values}
...
def test_expanduser_tilde_is_not_a_fix(tmp_path):
    ...
    code = ("import os;"
            ...
            "print(os.path.expanduser('~konan'))")
