...
class TestLearningExporterData(unittest.TestCase):
    ...
    def test_empty_data_on_fresh_db(self):
        """Real DB: smoke-test that the exporter runs without crashing."""
        entry = export_dojo_learnings(TEST_INSTANCE_ID, days=7)
        # Real DB may have accumulated data — just verify shape, not count
        self.assertIsInstance(entry["total_learnings"], int)
        self.assertIsInstance(entry["learnings"], list)
        self.assertIsInstance(entry["by_type"], dict)
...
class TestLearningExporterWithData(unittest.TestCase):
    ...
    def test_exports_3_learnings(self):
        ...
        try:
            entry = export_dojo_learnings(TEST_INSTANCE_ID, days=365)
        finally:
