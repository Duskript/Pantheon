...
class TestGateD1EquivalentOutput(unittest.TestCase):
    ...
    def test_tick_covers_all_six_old_cron_outputs(self):
        ...
        expected_steps = {
            "gather", "extract", "finalize", "analyze", "improve",
            "brief", "export", "verify",
        }
...
class TestStepExtract(unittest.TestCase):
    ...
    def test_runs_tier_a_extraction(self):
        ...
        self.assertIn("residual_events", result)
        self.assertIn("would_call_batches", result)
        self.assertIsInstance(result["residual_events"], int)
        self.assertIsInstance(result["would_call_batches"], int)
        self.assertGreaterEqual(result["residual_events"], 0)
        self.assertGreaterEqual(result["would_call_batches"], 0)
    ...
    def test_dry_run_does_not_modify_db(self):
        ...
        try:
            has_table = con.execute(
                "SELECT 1 FROM sqlite_master WHERE type='table' AND name='cold_events'"
            ).fetchone()
            if not has_table:
                self.skipTest("no cold_events table")
