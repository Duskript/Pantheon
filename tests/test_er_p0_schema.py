...
class TestER_P0_Gate(unittest.TestCase):
    ...
    def test_migrate_after_seed_preserves_data(self) -> None:
        ...
        try:
            conn.execute(
                "INSERT OR REPLACE INTO entity_types (id, description) VALUES (?, ?)",
                ("person", "A natural person"),
            )
            conn.commit()
        finally:
...
class TestER_P0_Contract(unittest.TestCase):
    ...
    def test_entity_types_supports_hierarchy(self) -> None:
        ...
        try:
            ...
            conn.execute(
                "INSERT OR IGNORE INTO entity_types (id, description, parent_type) "
                "VALUES (?, ?, ?)",
    ...
    def test_entity_aliases_stores_json_array(self) -> None:
        ...
        try:
            conn.execute(
                "INSERT OR IGNORE INTO entity_types (id) VALUES ('person')"
            )
    ...
    def test_relationships_bitemporal_columns(self) -> None:
        ...
        try:
            conn.executemany(
                "INSERT OR IGNORE INTO entity_types (id) VALUES (?)",
                [("person",), ("organization",)],
            )
            conn.executemany(
                "INSERT INTO entities (type_id, name) VALUES (?, ?)",
            ...
            conn.execute(
                "INSERT OR IGNORE INTO relationship_types (id, source_type, target_type) "
                "VALUES (?, ?, ?)",
                ("works_at", "person", "organization"),
            )
