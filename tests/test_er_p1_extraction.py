...
class TestTemporalSupersession(unittest.TestCase):
    def setUp(self):
        self.db_path = _isolated_db()
        migrate(self.db_path)
        self.conn = get_conn(self.db_path)
        self.conn.execute("INSERT OR IGNORE INTO entity_types (id, description) VALUES (?, ?)", ("person", "Person"))
        self.conn.execute(
        ...
        self.alice = self.conn.execute(
            "INSERT INTO entities (type_id, name) VALUES (?, ?)",
            ("person", "Alice"),
        ).lastrowid
        self.acme = self.conn.execute(
            "INSERT INTO entities (type_id, name) VALUES (?, ?)",
            ("organization", "Acme"),
        ).lastrowid
        self.conn.commit()
    ...
    def test_supersede_if_duplicate_closes_open_relationship(self):
        self.conn.execute(
            "INSERT INTO relationships (type_id, source_id, target_id, valid_from) VALUES (?, ?, ?, ?)",
            ("works_at", self.alice, self.acme, "2024-01-01 00:00:00"),
        )
        self.conn.commit()
        closed = supersede_if_duplicate(
            self.conn,
            "relationships",
            {"type_id": "works_at", "source_id": self.alice, "target_id": self.acme},
            {"valid_from": "2025-01-01 00:00:00"},
        )
        self.conn.commit()
        self.assertEqual(closed, [1])
        row = self.conn.execute(
            "SELECT valid_to FROM relationships WHERE id = 1",
        ).fetchone()
        self.assertIsNotNone(row["valid_to"])

    def test_backfill_relationship_temporal_bounds_sets_valid_to(self):
        self.conn.execute(
            "INSERT INTO relationships (type_id, source_id, target_id, valid_from) VALUES (?, ?, ?, ?)",
            ("works_at", self.alice, self.acme, "2024-01-01 00:00:00"),
        )
        self.conn.execute(
            "INSERT INTO relationships (type_id, source_id, target_id, valid_from) VALUES (?, ?, ?, ?)",
            ("works_at", self.alice, self.acme, "2025-01-01 00:00:00"),
        )
        self.conn.commit()
        result = backfill_relationship_temporal_bounds(self.conn)
        self.assertEqual(result["rows_updated"], 1)
        row1 = self.conn.execute(
            "SELECT valid_to FROM relationships WHERE id = 1",
        ).fetchone()
        row2 = self.conn.execute(
            "SELECT valid_to FROM relationships WHERE id = 2",
        ).fetchone()
        self.assertEqual(row1["valid_to"], "2025-01-01 00:00:00")
        self.assertIsNone(row2["valid_to"])
