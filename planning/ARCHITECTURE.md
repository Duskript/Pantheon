...
> **Canonical reference — Last updated: 2026-07-20**
...
│   ├── mcp-servers/                  ← Optional MCP adapters owned by Pantheon
│   │   └── olympus-btst-mcp/         ← BTST API stdio MCP wrapper
│   ├── hermes-dojo/                  ← Skill crystallization scripts + references
│   └── shared/
...
### 2.10 Optional MCP Adapters

**Location:** `~/pantheon/mcp-servers/`

Pantheon-owned MCP adapters expose local or adjacent systems to Hermes through the native MCP client without committing those systems' application data. The current adapter is `olympus-btst-mcp`, a stdio server wrapping BTST CMS/blog/media/forms/kanban/comment APIs behind `BTST_API_URL`.

### 2.11 Hermes Dojo and Skill Crystallization

**Location:** `~/pantheon/hermes-dojo/`

Hermes Dojo is the reusable learning loop around skills: scripts mine `~/.hermes/ichor.db` for high-signal crystallization candidates and recurring failure trajectories. Runtime logs, SQLite databases, temporary candidates, and generated batch-note markdown are excluded from the core repo; only reusable scripts and stable references ship.

### 2.12 Optional Phone Gateway

**Source:** `scripts/phone-daemon.py` and `god-packages/shared-skills/pantheon-phone-gateway/`

The phone gateway is an optional Android integration. The daemon uses ADB to poll notifications into local JSONL queues, and the shared skill defines the procedure for active phone control via ADB/uiautomator2 structured UI hierarchy. Runtime state is kept in `phone-daemon/` and ignored.
